...
"""Pour la prochaine fois :
        --> fonction vague d'apparition des ennemies ( 5 vagues comportant
            chacune 10 spawn points)
        --> affichage du score et de la vie du joueur
            sur le panneau jaune, avec un cercle rouge et le nb de pts de vie dedans

        --> ecran de game over si le joueur est mort"""
# Created:     05/08/2016
...
class Game :
      ...
      def new(self) :
          ...
          self.bullet_player = pg.sprite.Group()
      ...
      def run(self) :
          ...
          while self.playing:
            ...
            self.update()
            self.draw()
            print( self.player.life, self.player.score )
          pass
      ...
      def events(self) :
          ...
          hit_player =( pg.sprite.spritecollideany(self.player,self.mobs_list, False) or pg.sprite.spritecollideany(self.player,self.meteor_list, False))
          hit_mobs = pg.sprite.groupcollide(self.bullet_player,self.mobs_list, True,True)
          hit_meteor = pg.sprite.groupcollide(self.bullet_player,self.meteor_list, True,True)
          ...
          if hit_meteor :
             self.Spawn_meteor(1)
             self.player.score += point_meteor
          if hit_mobs :
             self.player.score += point_ennemies
      ...
      def menu(self):
          ...
          self.draw_text(ENTER, 18, WHITE, width/2 ,  height/2 )
          self.draw_text(CREDIT_1, 15, WHITE, width/2.85 ,  height -40 )
          self.draw_text(CREDIT_2, 15, WHITE, width/5 ,  height -30 )
          pg.display.flip()
          self.press_key()
          pass
      ...
      def Spawn_meteor(self,nb_meteor) :
         for i in range (nb_meteor):
              self.meteor = Meteor()
